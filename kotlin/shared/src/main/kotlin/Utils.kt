...
object UlitsMultiPlatform {
    inline fun <reified T> event(value: T) {
...
internal inline fun Packets.Contact.withContact(
    bot: Bot, errPos: String = "Packets.Contact.withContact", block: (Contact) -> String
): String = when (type) {
    1 -> withFriend(
        bot, "找不到对应好友，位置${errPos}，id:${id}"
    ) { block(it) }

    2 -> withGroup(
        bot, "找不到对应群组，位置${errPos}，id:${id}"
    ) { block(it) }

    3 -> withMember(
        bot, "找不到对应群组，位置${errPos}，id:${id}", "找不到对应群成员，位置${errPos}，id:${id}, gid:${groupId}"
    ) { _, member ->
        block(member)
    }
...
internal suspend inline fun <reified E : MessageEvent> nextMessage(
    time: Long, halt: Boolean, p: EventPriority = EventPriority.HIGHEST, crossinline filter: (E) -> Boolean
): MessageChain {
    val exec = suspend {
